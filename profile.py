...
Instructions:
Wait for the profile instance to start. The KIND cluster will already be setup
or be in the setup phase. Check setup progress after SSH-ing in:
  tail -f /var/tmp/kserve-setup.log
  cat  /var/tmp/kserve-setup.status

This profile installs the full stack including KEDA and Prometheus.
...
pc.defineParameter(
    ...
    True,
    longDescription=(
        "If True, startup.sh runs unattended on first boot and installs the full "
        "KServe LLMInferenceService stack with KEDA and Prometheus. "
        "Set False to SSH in and run scripts/cloudlab_setup.sh yourself."
    ),
)
...
if params.run_setup:
    _cmd = (
        "sudo bash {repo}/startup.sh"
        " >> /var/tmp/kserve-setup.log 2>&1"
        " && echo done > /var/tmp/kserve-setup.status"
        " || echo failed > /var/tmp/kserve-setup.status"
